...
def login_user(response: Response, user_data: User) -> RedirectResponse:
    return login(response=response, user_data=user_data)


@router.post("/logout", tags=["Login"])
def logout_user(response: Response, user_data: User = Depends(get_current_user)) -> dict[str, str]:
    response.delete_cookie(key="user_access_token")
    return {'message': 'Пользователь успешно вышел из системы.'}


@router.get("/appsec", tags=["AppSec"])
def get_appsec(key: str = 'practice_main', user_data: User = Depends(get_current_user)) -> dict[str, str]:
